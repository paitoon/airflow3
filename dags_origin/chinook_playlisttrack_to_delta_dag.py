...
with DAG(
    dag_id="chinook_playlisttrack_to_delta_dag",
    start_date=datetime(2025, 1, 1),
    schedule=None,
    catchup=False,
    dagrun_timeout=timedelta(minutes=60),
    tags=["chinook", "playlisttrack", "sqlite", "csv", "delta", "bronze", "etl", "dq"],
    ...
    validate_csv = BashOperator(
        ...
        bash_command="test -s /data/chinook_csv/PlaylistTrack.csv && echo '[VALIDATE_CSV_OK] /data/chinook_csv/PlaylistTrack.csv'",
    ...
    validate_cleansed_data = BashOperator(
        ...
        bash_command=(
            "test -d /data/chinook_cleansed/PlaylistTrack && "
            "test -s /data/chinook_metadata/PlaylistTrack_dq_report.json && "
            "echo '[VALIDATE_CLEANSED_OK] /data/chinook_cleansed/PlaylistTrack'"
        ),
        execution_timeout=timedelta(minutes=2),
    )
