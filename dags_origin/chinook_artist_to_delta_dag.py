...
with DAG(
    dag_id="chinook_artist_to_delta_dag",
    start_date=datetime(2025, 1, 1),
    schedule=None,
    catchup=False,
    dagrun_timeout=timedelta(minutes=60),
    tags=["chinook", "artist", "sqlite", "csv", "delta", "bronze", "etl", "dq"],
    ...
    validate_csv = BashOperator(
        ...
        bash_command="test -s /data/chinook_csv/Artist.csv && echo '[VALIDATE_CSV_OK] /data/chinook_csv/Artist.csv'",
    ...
    validate_cleansed_data = BashOperator(
        ...
        bash_command=(
            "test -d /data/chinook_cleansed/Artist && "
            "test -s /data/chinook_metadata/Artist_dq_report.json && "
            "echo '[VALIDATE_CLEANSED_OK] /data/chinook_cleansed/Artist'"
        ),
        execution_timeout=timedelta(minutes=2),
    )
