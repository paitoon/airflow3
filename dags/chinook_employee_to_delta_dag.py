...
with DAG(
    dag_id="chinook_employee_to_delta_dag",
    start_date=datetime(2025, 1, 1),
    schedule=None,
    catchup=False,
    dagrun_timeout=timedelta(minutes=60),
    tags=["chinook", "employee", "sqlite", "csv", "delta", "bronze", "etl", "dq"],
    ...
    validate_csv = BashOperator(
        ...
        bash_command="test -s /data/chinook_csv/Employee.csv && echo '[VALIDATE_CSV_OK] /data/chinook_csv/Employee.csv'",
    ...
    validate_cleansed_data = BashOperator(
        ...
        bash_command=(
            "test -d /data/chinook_cleansed/Employee && "
            "test -s /data/chinook_metadata/Employee_dq_report.json && "
            "echo '[VALIDATE_CLEANSED_OK] /data/chinook_cleansed/Employee'"
        ),
        execution_timeout=timedelta(minutes=2),
    )
