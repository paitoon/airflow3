...
with DAG(
    dag_id="chinook_mediatype_to_delta_dag",
    start_date=datetime(2025, 1, 1),
    schedule=None,
    catchup=False,
    dagrun_timeout=timedelta(minutes=60),
    tags=["chinook", "mediatype", "sqlite", "csv", "delta", "bronze", "etl", "dq"],
    ...
    validate_csv = BashOperator(
        ...
        bash_command="test -s /data/chinook_csv/MediaType.csv && echo '[VALIDATE_CSV_OK] /data/chinook_csv/MediaType.csv'",
    ...
    validate_cleansed_data = BashOperator(
        ...
        bash_command=(
            "test -d /data/chinook_cleansed/MediaType && "
            "test -s /data/chinook_metadata/MediaType_dq_report.json && "
            "echo '[VALIDATE_CLEANSED_OK] /data/chinook_cleansed/MediaType'"
        ),
        execution_timeout=timedelta(minutes=2),
    )
