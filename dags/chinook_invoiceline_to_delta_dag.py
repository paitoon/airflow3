...
with DAG(
    dag_id="chinook_invoiceline_to_delta_dag",
    start_date=datetime(2025, 1, 1),
    schedule=None,
    catchup=False,
    dagrun_timeout=timedelta(minutes=60),
    tags=["chinook", "invoiceline", "sqlite", "csv", "delta", "bronze", "etl", "dq"],
    ...
    wait_for_invoice = ExternalTaskSensor(
        task_id="wait_for_invoice",
        external_dag_id="chinook_invoice_to_delta_dag",
    ...
    validate_csv = BashOperator(
        ...
        bash_command="test -s /data/chinook_csv/InvoiceLine.csv && echo '[VALIDATE_CSV_OK] /data/chinook_csv/InvoiceLine.csv'",
    ...
    validate_cleansed_data = BashOperator(
        ...
        bash_command=(
            "test -d /data/chinook_cleansed/InvoiceLine && "
            "test -s /data/chinook_metadata/InvoiceLine_dq_report.json && "
            "echo '[VALIDATE_CLEANSED_OK] /data/chinook_cleansed/InvoiceLine'"
        ),
        execution_timeout=timedelta(minutes=2),
    )
    ...
    start >> [wait_for_invoice, wait_for_track] >> export_sqlite_to_csv
